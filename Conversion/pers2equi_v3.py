...
def pers2equi(pers_img, fov, nrows, patch_size, erp_size, layer_name):
    ...
    img_erp = F.grid_sample(pers_img, new_grid, mode='bilinear', padding_mode='border', align_corners=True)
        ...
        # print('cos_c_mask: ',cos_c_mask.shape)
        # for i in range(46):
        #     plt.imshow(cos_c_mask[i])
        #     plt.show()
...
if __name__ == "__main__":
    ...
    pers = equi2pers(img_new, fov=fov, nrows=6, patch_size=ps)
    print(pers[0].shape)
    pers = pers[0]
    # pers = pers[0].transpose(0, 4,2,3,1).astype(np.uint8)
    # pers = F.unfold(pers[0], kernel_size=64, stride=64)
    # pers = pers.reshape(1, 3, 64, 64, -1)

    erp = pers2equi(pers, fov=fov, nrows=6, patch_size=ps, erp_size=(256, 512), layer_name='test')
    ...
    cv2.imwrite('test/interp_erp.png', img_erp_int)
